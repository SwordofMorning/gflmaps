...
const loadData = async () => {
  ...
  const loadTextTable = (file) => loadJsonFile(`https://raw.githubusercontent.com/randomqwerty/GFLData/main/ch/text/table/${file}`);
  ...
  const loaders = {
    ...
    "UI_TEXT": loadJsonFile(`./text/${config.langCode}/ui_text.json?v=1788836955`).then((result) => UI_TEXT = result),
    ...
    "INSTRUCTIONS": loadTextFile(`./text/${config.langCode}/instructions.html?v=1788836955`).then((result) => INSTRUCTIONS = result),
    
    "spot_images": Promise.all(spotPaths.map((path) => loadImageAsset(`spot/en/${path}`))),
  };
...
function drawmap(func){
    ...
    for(i in dspot){
      ...
      if (spotFilename) {
        const spot = loadedImageAssets[`spot/en/${spotFilename}.png`];
        if (spot) {
